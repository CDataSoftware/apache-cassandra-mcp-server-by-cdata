...
CData's Model Context Protocol (MCP) Server for Apache Cassandra

:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Apache Cassandra (beta)](https://www.cdata.com/download/download.aspx?sku=RCZK-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Apache Cassandra supported by the [CData JDBC Driver for Apache Cassandra](https://www.cdata.com/drivers/cassandra/jdbc).

CData JDBC Driver connects to Apache Cassandra by exposing them as relational SQL models.

This server wraps that driver and makes Apache Cassandra data available through a simple MCP interface, so LLMs can retrieve live information by asking natural language questions — no SQL required.
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/cassandra/download/jdbc](https://www.cdata.com/drivers/cassandra/download/jdbc)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Apache Cassandra\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Apache Cassandra/`
...
5. Create a `.prp` file for your JDBC connection (e.g. `apache-cassandra.prp`) using the following properties and format:
    * **Prefix** - a prefix to be used for the tools exposed
    * **ServerName** - a name for your server
    * **ServerVersion** - a version for your server
    * **DriverPath** - the full path to the JAR file for your JDBC driver
    * **DriverClass** - the name of the JDBC Driver Class (e.g. cdata.jdbc.cassandra.CassandraDriver)
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "apache_cassandra_get_tables",
        "arguments": {}
    }
}
```
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "apache_cassandra_get_columns",
        "arguments": {
            "table":  "Account"
        }
    }
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "apache_cassandra_run_query",
        "arguments": {
